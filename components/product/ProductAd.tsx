...
export default function ProductAd({
    ...
    return (
        <div class="container px-3 sm:px-0">
            <div class={`flex flex-col relative ${vertical && !isMobile && "sm:flex-row"} gap-3 ${highlight && productLikes >= 3 ? "bg-black border-2 border-transparent" : "bg-black border-2 border-white"} p-3 rounded-xl my-5 min-h-[320px] max-h-[320px]`}>
                <a 
                    href={url}
                ...
                <div class={`flex flex-col items-start w-full ${!vertical && "mt-2"}`}>
                    <h2 class=" inline-block max-w-full text-lg font-bold uppercase text-white truncate">{name}</h2>
